...
function onLoad() {
  if (document.getElementById('candidate_table')) {
    ...
    document.getElementById('candidate_count').value = localStorage.getItem(
      'election.candidate_count',
    );
    document.getElementById('list_name').value =
      localStorage.getItem('election.list_name');
    document.getElementById('list_owners').value = localStorage.getItem(
      'election.list_owners',
    );
...
function employeeCount(isExample) {
  return isExample
    ? exampleEmployeeCount
    : parseInt(localStorage.getItem('election.employee_count'));
}
...
function signaturesTable(isExample) {
  ...
  const tableCandidateSignatureBody = document.querySelector(
    '#signatures_candidate_id',
  );
  const caption = document.querySelector(
    '#signature_candidate_table > caption',
  );
  let caption_text = `
...
function candidates(isExample) {
  ...
  return (
    parseInt(localStorage.getItem('election.candidate_count')) ||
    2 * worksCouncil()
  );
}
